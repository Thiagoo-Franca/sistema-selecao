...
export default function useCourses() {
  ...
  const refreshCourses = () => api.get("cursos").then(({ data: { data } }) => setCourses(data))

  const memoizedCourses = useMemo(() => {
    if (query) {
      return courses.filter((course) => course.nome && course.nome.toLowerCase().includes(query.toLowerCase()))
    }
    return courses
  }, [courses, query])
